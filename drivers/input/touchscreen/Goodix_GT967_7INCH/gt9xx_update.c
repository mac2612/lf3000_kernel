...
#define CONFIG_FILE_PATH_2          "/data/_goodix_config_.cfg"     
#define CONFIG_FILE_PATH_1          "/sdcard/_goodix_config_.cfg"   
...
static s8 gup_update_config(struct i2c_client *client)
{
    ...
    if(ret<0)
    {
        GTP_ERROR("[update_cfg]Read config file failed. ret=%d",ret);
...
#endif
s32 gup_check_config_file(void *path)//struct i2c_client *client,u8* path)
{
	struct goodix_ts_data *ts = NULL;
	s32 ret = 0; 	
	got_file_flag = 0;
    
    	GTP_DEBUG1("[update_proc]Begin update config......");
    
    	ts = i2c_get_clientdata(i2c_connect_client);
	if (path)
    	{
      	  	GTP_DEBUG1("Config File path:%s, %d", (u8*)path, strlen(path));
        	update_msg.cfg_file = filp_open((u8*)path, O_RDONLY, 0);

        	if (IS_ERR(update_msg.cfg_file))
        	{
           	 	GTP_ERROR("Open config file(%s) error!", (u8*)path);
            		return FAIL;
        	}
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    else
    {
        ...
        //{
		    ...
		    GTP_DEBUG1("Not found the bin file");
            }
		    ...
		    GTP_DEBUG1("Not found the cfg file");
            }
...
s32 gup_update_proc(void *dir)
{
	...
	;//ret = gup_check_config_file(i2c_connect_client, (u8*)dir);
    if(FAIL == ret)
    {
...
u8 gup_init_config_proc(struct goodix_ts_data *ts,char* file_path)
{
    struct task_struct *thread = NULL;

    GTP_INFO("Ready to run update config thread.");
    thread = kthread_run(gup_check_config_file, (void*)file_path, "guitar_update_config");
    if (IS_ERR(thread))
    {
        GTP_ERROR("Failed to create update config thread.\n");
        return -1;
    }
...
//#if GTP_AUTO_UPDATE
u8 gup_init_update_proc(struct goodix_ts_data *ts,char* file_path)
{
    struct task_struct *thread = NULL;

    GTP_INFO("Ready to run update thread.");
    thread = kthread_run(gup_update_proc, (void*)file_path, "guitar_update");
    if (IS_ERR(thread))
    {
