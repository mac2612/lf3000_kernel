...
#ifdef CONFIG_NXP4330_LEAPFROG
static struct usbnet *ethdev = NULL;
#endif
...
#ifdef CONFIG_NXP4330_LEAPFROG
		} else if (ax772b_data->suspend) {
			ax772b_data->pw_enabled = 1;
			ax772b_data->Event = PHY_POWER_DOWN;
			ax772b_data->time_to_chk = jiffies;
		}
#endif
	}
 ...
 */
#ifdef CONFIG_NXP4330_LEAPFROG
/* report whether suspend was requested */
static ssize_t show_suspend(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct ax88772b_data *ax772b_data = (struct ax88772b_data *)ethdev->priv;

	return sprintf(buf, "%d\n", ax772b_data->suspend);
}
/* set suspend to requested value */
static ssize_t store_suspend(struct device *pdev, struct device_attribute *attr,
		                      const char *buf, size_t count)
{
	...
	ax772b_data->suspend = temp;
	if(ax772b_data->suspend) {
		devwarn(ethdev, "suspend requested");
		/* Power down PHY */
		ax8817x_write_cmd(ethdev, AX_CMD_SW_RESET, AX_SWRESET_IPPD, 0, 0, NULL);
	}
	else {
		/* Power up PHY */
		ax88772a_phy_powerup(ethdev);
	}

	return(count);
}
static DEVICE_ATTR(suspend, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH,
		show_suspend, store_suspend);

static struct attribute *suspend_attributes[] = {
	&dev_attr_suspend.attr,
	NULL
};

static struct attribute_group suspend_attr_group = {
	.attrs = suspend_attributes
};
#endif
...
#ifdef CONFIG_NXP4330_LEAPFROG
static struct platform_device asix_88772c_plat_device = {
	.name	= "asix-88772c",
	.id		= -1,
};

static int axix_probe(struct usb_interface *udev, const struct usb_device_id *prod)
{
	struct platform_device *pdev = &asix_88772c_plat_device;

	sysfs_create_group(&pdev->dev.kobj, &suspend_attr_group);

	return axusbnet_probe(udev, prod);
}

static void ax_remove(struct usb_interface *intf)
{
	struct platform_device *pdev = &asix_88772c_plat_device;

	sysfs_remove_group(&pdev->dev.kobj, &suspend_attr_group);

	axusbnet_disconnect(intf);
...
{
#ifdef CONFIG_NXP4330_LEAPFROG
	printk(KERN_WARNING, "plat: register asix 88772C plat monitor\n");
	platform_device_register(&asix_88772c_plat_device);
#endif
